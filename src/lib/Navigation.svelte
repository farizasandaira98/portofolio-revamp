...
<nav class="fixed inset-x-0 top-0 z-[9999]">
  <!-- Desktop Nav: centered at top -->
  <div class="hidden md:flex fixed top-8 left-1/2 transform -translate-x-1/2 glass-effect {navOpacity} {borderOpacity} rounded-full px-8 py-4 items-center space-x-8 backdrop-blur-md">
  ...
  <button class="md:hidden fixed top-8 left-8 w-12 h-12 {navOpacity} backdrop-blur-md border {borderOpacity} rounded-full flex items-center justify-center text-white hover:bg-white/25 transition-all duration-300 focus:outline-none z-[9999]" on:click={toggleMenu} aria-label="Open navigation menu">
  ...
  {#if menuOpen}
    <div class="fixed top-20 left-4 w-56 glass-effect {navOpacity} {borderOpacity} backdrop-blur-md rounded-2xl shadow-xl py-4 z-[9998] flex flex-col md:hidden animate-fade-in">
    ...
    <!-- Overlay -->
    <div 
      class="fixed inset-0 z-[9997] md:hidden" 
      on:click={closeMenu}
