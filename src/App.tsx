	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "ArrowDown" || (e.key === " " && !isFormField)) {
				...
				e.preventDefault();
				const prevSection =
					activeSection > 0 ? activeSection - 1 : activeSection;
	...
	useEffect(() => {
		...
		const handleWheel = (e: WheelEvent) => {
			...
			const scrollableContainer =
				currentSectionElement.querySelector(".section-scroll-container") ||
		...
		const handleTouchStart = (e: TouchEvent) => {
			...
			if (currentSectionElement) {
				const scrollableContainer =
					currentSectionElement.querySelector(".section-scroll-container") ||
					currentSectionElement;
				_initialScrollTop = scrollableContainer.scrollTop;
			}
		};
	...
	return (
		<div className="h-screen overflow-hidden bg-background text-foreground relative">
			...
			<div className="fixed inset-0 pointer-events-none z-[-1]">
				{[...Array(12)].map((_, i) => (
					<motion.div
						key={`app-decoration-${i}-${Math.random()}`}
